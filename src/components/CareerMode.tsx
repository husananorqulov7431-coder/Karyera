...
import { sfxClick, sfxCardFlip, sfxWalkoutReveal, sfxWhistle } from '../utils/audio';
...
interface CareerModeProps {
  ...
  onTriggerEpicWalkout?: (player: Player) => void;
  onRewardGp?: (amount: number) => void;
}
...
export const CareerMode: React.FC<CareerModeProps> = ({
  ...
  onAddPlayerToSquad,
  userDisplayName,
  onTriggerEpicWalkout,
  onRewardGp
}) => {
  // Ism kiritish shart! Boshlang'ich qiymat bo'sh bo'ladi, foydalanuvchi albatta o'z ismini yoki xohlagan ismni kiritishi shart
  ...
  const handleStartCareer = () => {
    const trimmedName = customCareerName.trim();
    if (!trimmedName || trimmedName.length < 2) {
      setNameError("⚠️ Futbolchi ismini kiritish majburiy! Iltimos, o‘z ismingizni yoki yangi futbolchi ismini yozing (kamida 2 ta harf).");
      sfxWhistle();
      speakText("Futbolchi ismini kiritish majburiy. Iltimos, ism yozing.", true);
    ...
    const playerName = customCareerName.trim();
  ...
  const handleProceedToResult = () => {
    ...
    if (onRewardGp) onRewardGp(100);
    if (careerResult.player.ovr >= 89 && onTriggerEpicWalkout) {
      onTriggerEpicWalkout(careerResult.player);
    }
        ...
        >
          <div>
            ...
            <p className="text-xs sm:text-sm text-slate-300 mt-1">
              Futbolchi karyerasi siz kiritgan nom va tanlangan chempionat statistikasi asosida quriladi (soxta ismlar generatsiya qilinmaydi).
            </p>
          </div>

          {/* Player Custom Name Input (Majburiy) */}
          <div className="flex flex-col gap-2">
            <div className="flex items-center justify-between">
              <span className="text-xs font-black uppercase tracking-wider text-slate-200 flex items-center gap-1.5">
                <span>Futbolchining Ismi (O‘zingizning Ismingiz)</span>
                <span className="text-red-400 font-bold">* majburiy</span>
              </span>
              {customCareerName.trim().length >= 2 && (
                <span className="text-[11px] text-emerald-400 font-semibold flex items-center gap-1">
                  <Check className="w-3.5 h-3.5 text-emerald-400" />
                  <span>Ism kiritildi ({customCareerName.trim()})</span>
                </span>
              )}
            </div>
            <input
              ref={nameInputRef}
              type="text"
              placeholder="Masalan: Sardor Rashidov, Jasur Qodirov yoki o‘z ismingizni kiriting"
              value={customCareerName}
              onChange={(e) => {
                setCustomCareerName(e.target.value);
                if (nameError && e.target.value.trim().length >= 2) {
                  setNameError(null);
                }
              }}
              className={`w-full px-4 py-3 rounded-2xl bg-white/[0.06] border text-white placeholder-slate-500 text-sm font-bold focus:outline-none transition-all ${
                nameError
            ...
            />
            {nameError && (
              <div className="flex items-center gap-1.5 text-xs text-red-400 font-bold mt-0.5">
                <AlertCircle className="w-4 h-4 text-red-400 shrink-0" />
                <span>{nameError}</span>
              </div>
            )}
            <p className="text-[11px] text-slate-400">
              Lotereyada qatnashish uchun o‘z ismingizni yoki istalgan futbolchi nomini yozing. Ismsiz boshlash taqiqlanadi.
            </p>
          </div>
          ...
          <button
            ...
            className={`w-full py-4 rounded-2xl font-black text-base uppercase tracking-wider transition-all cursor-pointer shadow-xl ${
              !customCareerName.trim()
                ? 'bg-gradient-to-r from-amber-500/80 to-orange-500/80 hover:from-amber-400 hover:to-orange-400 text-slate-950 shadow-amber-500/10'
                : 'bg-gradient-to-r from-amber-400 via-amber-500 to-orange-500 hover:from-amber-300 hover:to-orange-400 text-slate-950 shadow-amber-500/25 hover:scale-101'
            }`}
          >
            {customCareerName.trim().length >= 2
              ? `Karyera Taqdirini Boshlash (${customCareerName.trim()})`
              : '✍️ Avval Futbolchi Ismini Kiriting (Shart)'}
          </button>
        </motion.div>
      )}
