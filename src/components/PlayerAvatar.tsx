...
export const PlayerAvatar: React.FC<PlayerAvatarProps> = ({ avatar, size = 120, className = '' }) => {
  const safeAvatar: AvatarConfig = {
    ...DEFAULT_AVATAR,
    ...(avatar || {})
  };

  const skinTone = safeAvatar.skinTone || DEFAULT_AVATAR.skinTone;
  const hairStyle = safeAvatar.hairStyle || DEFAULT_AVATAR.hairStyle;
  const hairColor = safeAvatar.hairColor || DEFAULT_AVATAR.hairColor;
  const facialHair = safeAvatar.facialHair || DEFAULT_AVATAR.facialHair;
  const kitPrimaryColor = safeAvatar.kitPrimaryColor || DEFAULT_AVATAR.kitPrimaryColor;
  const kitSecondaryColor = safeAvatar.kitSecondaryColor || DEFAULT_AVATAR.kitSecondaryColor;
  const likenessName = safeAvatar.likenessName || DEFAULT_AVATAR.likenessName;
