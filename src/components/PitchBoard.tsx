...
interface PitchBoardProps {
  ...
  onMoveToReserves?: (player: Player) => void;
}
...
export const PitchBoard: React.FC<PitchBoardProps> = ({
  ...
  // Roster Category & Filter State (Zaxira vs Qolgan Futbolchilar / Epic vs Highlight vs Oddiy)
  const [activeRosterTab, setActiveRosterTab] = useState<'bench' | 'reserves'>('bench');
  const [rosterRarityFilter, setRosterRarityFilter] = useState<'all' | 'epic' | 'highlight' | 'standard'>('all');

  // Picker Modal Filters
  const [pickerRarityFilter, setPickerRarityFilter] = useState<'all' | 'epic' | 'highlight' | 'standard'>('all');
  const [pickerRoleFilter, setPickerRoleFilter] = useState<'ALL' | 'FW' | 'MF' | 'DF' | 'GK'>('ALL');
    ...
    <div className="w-full flex flex-col gap-4 max-w-7xl mx-auto pb-12">
        ...
        <div className="lg:col-span-8 p-4 rounded-3xl bg-slate-900/90 border border-white/10 backdrop-blur-xl flex flex-wrap items-center justify-between gap-4 shadow-xl">
          ...
          {onRestoreStamina && (
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-300 border border-emerald-500/40 text-xs font-black transition-all cursor-pointer shadow-sm active:scale-95"
              title="Barcha o‘yinchilarning jonini (stamina) 100% ga to‘ldirish"
            >
              <Zap className="w-3.5 h-3.5 fill-current text-emerald-400" />
              <span className="hidden sm:inline">Jonni Tiklash</span>
            </button>
          )}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-4 items-start">
                      ...
                      {player ? (
                        <>
                          ...
                          <div className="w-full text-center truncate px-0.5">
                            ...
                            {/* Stamina (Jon) indicator */}
                            <div className="w-full h-1 rounded-full bg-slate-800/90 overflow-hidden mt-0.5 border border-black/40" title={`Jon (Stamina): ${player.stamina ?? 100}%`}>
                              <div
                                className={`h-full rounded-full transition-all ${
                                  (player.stamina ?? 100) > 60
                                    ? 'bg-emerald-400'
                                    : (player.stamina ?? 100) > 30
                                    ? 'bg-amber-400'
        ...
        <div className="xl:col-span-4 flex flex-col gap-4">
          ...
          {/* ROSTER MANAGEMENT: BENCH vs RESERVES (Zaxira va Qolgan Futbolchilar) */}
          <div className="p-4 rounded-3xl bg-slate-900/90 border border-white/10 backdrop-blur-xl flex flex-col gap-3 shadow-xl">
            {/* Top Minimal Tab Switcher */}
            <div className="flex items-center justify-between gap-2 border-b border-white/10 pb-2.5">
              <div className="flex items-center gap-1.5 p-1 rounded-2xl bg-black/40 border border-white/5">
                <button
                  onClick={() => {
                    sfxClick();
                    setActiveRosterTab('bench');
                  }}
                  className={`px-3 py-1.5 rounded-xl text-xs font-black transition-all cursor-pointer flex items-center gap-1.5 ${
                    activeRosterTab === 'bench'
                      ? 'bg-cyan-500 text-slate-950 shadow-md shadow-cyan-500/20'
                      : 'text-slate-400 hover:text-white'
                  }`}
                >
                  <Users className="w-3.5 h-3.5" />
                  <span>Zaxira ({bench.length})</span>
                </button>

                <button
                  onClick={() => {
                    sfxClick();
                    setActiveRosterTab('reserves');
                  }}
                  className={`px-3 py-1.5 rounded-xl text-xs font-black transition-all cursor-pointer flex items-center gap-1.5 ${
                    activeRosterTab === 'reserves'
                      ? 'bg-amber-400 text-slate-950 shadow-md shadow-amber-400/20'
                      : 'text-slate-400 hover:text-white'
                  }`}
                >
                  <Layers className="w-3.5 h-3.5" />
                  <span>Qolgan Futbolchilar ({reserves.length})</span>
                </button>
              </div>

              <span className="text-[10px] text-slate-400 hidden sm:inline">
                {activeRosterTab === 'bench' ? 'O‘yin uchun faol zaxira' : 'Klub ro‘yxatidagi o‘yinchilar'}
              </span>
            </div>

            {/* Sub-Filters: Epic, Highlight, Standard */}
            {(() => {
              const currentRosterList = activeRosterTab === 'bench' ? bench : reserves;
              const epicCount = currentRosterList.filter(p => (p.ovr || 0) >= 89).length;
              const highlightCount = currentRosterList.filter(p => (p.ovr || 0) >= 84 && (p.ovr || 0) < 89).length;
              const standardCount = currentRosterList.filter(p => (p.ovr || 0) < 84).length;

              const filteredList = currentRosterList.filter(p => {
                if (rosterRarityFilter === 'epic') return (p.ovr || 0) >= 89;
                if (rosterRarityFilter === 'highlight') return (p.ovr || 0) >= 84 && (p.ovr || 0) < 89;
                if (rosterRarityFilter === 'standard') return (p.ovr || 0) < 84;
                return true;
              });
              ...
              return (
                <>
                  <div className="flex items-center gap-1.5 overflow-x-auto pb-1 text-[11px]">
                    <button
                      onClick={() => { sfxClick(); setRosterRarityFilter('all'); }}
                    ...
                    >
                      Barchasi ({currentRosterList.length})
                    </button>
                    <button
                      onClick={() => { sfxClick(); setRosterRarityFilter('epic'); }}
                      className={`px-2.5 py-1 rounded-xl font-bold transition-all cursor-pointer whitespace-nowrap flex items-center gap-1 ${
                        rosterRarityFilter === 'epic'
                          ? 'bg-amber-400 text-slate-950'
                          : 'bg-amber-400/10 text-amber-300 hover:bg-amber-400/20'
                      }`}
                    >
                      <span>👑 Epic</span>
                      <span className="opacity-80">({epicCount})</span>
                    </button>
                    <button
                      onClick={() => { sfxClick(); setRosterRarityFilter('highlight'); }}
                      className={`px-2.5 py-1 rounded-xl font-bold transition-all cursor-pointer whitespace-nowrap flex items-center gap-1 ${
                        rosterRarityFilter === 'highlight'
                          ? 'bg-cyan-400 text-slate-950'
                          : 'bg-cyan-400/10 text-cyan-300 hover:bg-cyan-400/20'
                      }`}
                    >
                      <span>💎 Highlight</span>
                      <span className="opacity-80">({highlightCount})</span>
                    </button>
                    <button
                      onClick={() => { sfxClick(); setRosterRarityFilter('standard'); }}
                      className={`px-2.5 py-1 rounded-xl font-bold transition-all cursor-pointer whitespace-nowrap flex items-center gap-1 ${
                        rosterRarityFilter === 'standard'
                          ? 'bg-slate-400 text-slate-950'
                          : 'bg-white/5 text-slate-400 hover:text-white'
                      }`}
                    >
                      <span>⚽ Oddiy</span>
                      <span className="opacity-80">({standardCount})</span>
                    </button>
                  </div>

                  {filteredList.length === 0 ? (
                    <div className="p-6 rounded-2xl bg-white/[0.02] border border-dashed border-white/10 text-center">
                      <p className="text-xs text-slate-400">Ushbu toifada futbolchi mavjud emas</p>
                    </div>
                  ) : (
                    <div className="grid grid-cols-1 gap-2 max-h-[300px] overflow-y-auto pr-1">
                      {filteredList.map(player => {
                        const isSelectedBench = swapSourceBenchPlayer?.id === player.id;
                        const isEpic = (player.ovr || 0) >= 89;
                        const isHighlight = (player.ovr || 0) >= 84 && (player.ovr || 0) < 89;

                        return (
                          <div
                            key={`roster_${player.id}`}
                            className={`p-2 rounded-2xl border transition-all flex items-center justify-between gap-2 ${
                              isSelectedBench
                                ...
                                : isHighlight
                                ? 'bg-cyan-500/5 border-cyan-500/30 hover:border-cyan-400/60'
                          ...
                          >
                                ...
                                <div className="flex items-center gap-1.5">
                                  ...
                                  <span
                                    className={`text-[10px] font-black ${
                                      isEpic ? 'text-amber-400' : isHighlight ? 'text-cyan-300' : 'text-slate-300'
                                    }`}
                                  >
                                    {player.ovr}
                                  </span>
                                  {isEpic && (
                                    <span className="text-[8px] font-black uppercase px-1 py-0.2 rounded bg-amber-400/20 text-amber-300 border border-amber-400/40">
                                      EPIC
                                    </span>
                                  )}
                                </div>
                            ...
                            {/* Action Buttons: Tushirish / Zaxiraga o'tkazish */}
                            <div className="flex items-center gap-1.5 shrink-0">
                              {activeRosterTab === 'reserves' && onMoveToBench && (
                                <button
                                  onClick={() => {
                                    sfxClick();
                                    onMoveToBench(player);
                                  }}
                                  title="O'yin zaxirasiga qo'shish"
                                  className="px-2 py-1 rounded-xl text-[10px] font-bold bg-white/10 hover:bg-white/20 text-slate-200 transition-colors cursor-pointer"
                                >
                                  + Zaxiraga
                                </button>
                              )}

                              {activeRosterTab === 'bench' && onMoveToReserves && (
                                <button
                                  onClick={() => {
                                    sfxClick();
                                    onMoveToReserves(player);
                                  }}
                                  title="Klub ro'yxatiga qaytarish"
                                  className="px-2 py-1 rounded-xl text-[10px] font-bold bg-white/10 hover:bg-white/20 text-slate-200 transition-colors cursor-pointer"
                                >
                                  Klubga
                                </button>
                              ...
                              >
                                ...
                                <span>{isSelectedBench ? 'Tanlandi' : 'Maydonga'}</span>
                              </button>
                            </div>
                          </div>
                        );
            ...
            >
              <div className="flex justify-between items-center border-b border-white/10 pb-3">
                <div>
                  ...
                  <p className="text-xs text-slate-400">Epic, Highlight va Oddiy futbolchilar ro‘yxati</p>
                </div>
                <button
                  onClick={() => setPickerModalOpen(false)}
              ...
              {/* Filter Controls: Search, Rarity, Role */}
              <div className="space-y-2">
                <input
                  type="text"
                  value={pickerSearch}
                  onChange={e => setPickerSearch(e.target.value)}
                  placeholder="Futbolchi ismi yoki klubi bo‘yicha qidiruv..."
                  className="w-full px-3.5 py-2 rounded-xl bg-black/40 border border-white/10 text-xs text-white placeholder-slate-500 outline-none focus:border-cyan-400"
                />

                {/* Rarity Tabs */}
                {(() => {
                  const epics = availablePlayers.filter(p => (p.ovr || 0) >= 89).length;
                  const highlights = availablePlayers.filter(p => (p.ovr || 0) >= 84 && (p.ovr || 0) < 89).length;
                  const standards = availablePlayers.filter(p => (p.ovr || 0) < 84).length;

                  return (
                    <div className="flex items-center gap-1.5 overflow-x-auto text-[11px] pb-1">
                      <button
                        onClick={() => setPickerRarityFilter('all')}
                        className={`px-3 py-1 rounded-xl font-bold cursor-pointer whitespace-nowrap transition-all ${
                          pickerRarityFilter === 'all'
                            ? 'bg-cyan-500 text-slate-950 font-black'
                            : 'bg-white/5 text-slate-400 hover:text-white'
                        }`}
                      >
                        Barchasi ({availablePlayers.length})
                      </button>

                      <button
                        onClick={() => setPickerRarityFilter('epic')}
                        className={`px-3 py-1 rounded-xl font-bold cursor-pointer whitespace-nowrap flex items-center gap-1 transition-all ${
                          pickerRarityFilter === 'epic'
                            ? 'bg-amber-400 text-slate-950 font-black'
                            : 'bg-amber-400/10 text-amber-300 hover:bg-amber-400/20'
                        }`}
                      >
                        <span>👑 Epiclar</span>
                        <span className="opacity-80">({epics})</span>
                      </button>

                      <button
                        onClick={() => setPickerRarityFilter('highlight')}
                        className={`px-3 py-1 rounded-xl font-bold cursor-pointer whitespace-nowrap flex items-center gap-1 transition-all ${
                          pickerRarityFilter === 'highlight'
                            ? 'bg-cyan-400 text-slate-950 font-black'
                            : 'bg-cyan-400/10 text-cyan-300 hover:bg-cyan-400/20'
                        }`}
                      >
                        <span>💎 Highlight</span>
                        <span className="opacity-80">({highlights})</span>
                      </button>

                      <button
                        onClick={() => setPickerRarityFilter('standard')}
                        className={`px-3 py-1 rounded-xl font-bold cursor-pointer whitespace-nowrap flex items-center gap-1 transition-all ${
                          pickerRarityFilter === 'standard'
                            ? 'bg-slate-300 text-slate-950 font-black'
                            : 'bg-white/5 text-slate-400 hover:text-white'
                        }`}
                      >
                        <span>⚽ Oddiylar</span>
                        <span className="opacity-80">({standards})</span>
                      </button>
                    </div>
                  );
                })()}
                ...
                <div className="flex items-center gap-1 overflow-x-auto text-[10px]">
                  {(['ALL', 'FW', 'MF', 'DF', 'GK'] as const).map(roleCat => (
                    <button
                      key={roleCat}
                      onClick={() => setPickerRoleFilter(roleCat)}
                      className={`px-2.5 py-0.5 rounded-lg font-bold transition-all cursor-pointer ${
                        pickerRoleFilter === roleCat
                          ? 'bg-white/30 text-white font-black'
                          : 'bg-white/5 text-slate-400 hover:text-white'
                      }`}
                    >
                      {roleCat === 'ALL' ? 'Barcha Pozitsiyalar' : roleCat}
                    </button>
                  ))}
                </div>
              </div>
              ...
              {(() => {
                ...
                const filtered = availablePlayers.filter(p => {
                  // Rarity filter
                  if (pickerRarityFilter === 'epic' && (p.ovr || 0) < 89) return false;
                  if (pickerRarityFilter === 'highlight' && ((p.ovr || 0) < 84 || (p.ovr || 0) >= 89)) return false;
                  if (pickerRarityFilter === 'standard' && (p.ovr || 0) >= 84) return false;

                  // Role filter
                  if (pickerRoleFilter !== 'ALL') {
                    if (pickerRoleFilter === 'GK' && p.role !== 'GK') return false;
                    if (pickerRoleFilter === 'DF' && !['CB', 'LB', 'RB'].includes(p.role)) return false;
                    if (pickerRoleFilter === 'MF' && !['DMF', 'CMF', 'AMF', 'LMF', 'RMF'].includes(p.role)) return false;
                    if (pickerRoleFilter === 'FW' && !['CF', 'SS', 'LWF', 'RWF'].includes(p.role)) return false;
                  }

                  // Search query
                  if (searchLower) {
                    const matchName = (p.name || '').toLowerCase().includes(searchLower);
                    const matchClub = (p.club || '').toLowerCase().includes(searchLower);
                ...
                if (filtered.length === 0) {
                  return (
                    <div className="p-8 text-center text-xs text-slate-400">
                      Ushbu filtr bo‘yicha futbolchi topilmadi
                    </div>
                  );
                }

                return (
                  <div className="flex-1 overflow-y-auto grid grid-cols-1 sm:grid-cols-2 gap-2 pr-1 max-h-[360px]">
                    {filtered.map((player, idx) => {
                      ...
                      return (
                        <button
                          ...
                          className={`flex items-center gap-3 p-2.5 rounded-2xl border transition-all text-left cursor-pointer group ${
                            isEpic
                              ? 'bg-amber-500/10 border-amber-400/40 hover:border-amber-400 hover:bg-amber-500/20'
                              : isHighlight
                              ? 'bg-cyan-500/10 border-cyan-400/40 hover:border-cyan-400 hover:bg-cyan-500/20'
                              : 'bg-white/[0.05] hover:bg-white/[0.12] border-white/10 hover:border-white/30'
                          }`}
                        >
                          <PlayerAvatar avatar={player.avatar} size={38} />
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center justify-between">
                              ...
                              <span
                                className={`font-black text-xs ${
                                  isEpic ? 'text-amber-400' : isHighlight ? 'text-cyan-400' : 'text-slate-300'
                                }`}
                              >
                                {player.ovr}
                              </span>
