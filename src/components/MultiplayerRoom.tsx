...
export const MultiplayerRoom: React.FC<MultiplayerRoomProps> = ({
  ...
  const initPeer = () => {
    ...
    peer.on('disconnected', () => {
      // Disconnected from PeerJS cloud signaling server (e.g. idle timeout)
      // Existing P2P WebRTC data connection remains intact, but reconnect to server for new peers
      if (isMountedRef.current && peer && !peer.destroyed) {
        setTimeout(() => {
          if (isMountedRef.current && peer && !peer.destroyed && peer.disconnected) {
            try {
              peer.reconnect();
            } catch {}
          }
    ...
    peer.on('error', (err: any) => {
      ...
      // ID already in use on public server - fallback to random ID
      if (errType === 'unavailable-id') {
        try {
          ...
          fallbackPeer.on('disconnected', () => {
            if (isMountedRef.current && !fallbackPeer.destroyed) {
              setTimeout(() => {
                if (isMountedRef.current && !fallbackPeer.destroyed && fallbackPeer.disconnected) {
                  try {
                    fallbackPeer.reconnect();
                  } catch {}
                }
              }, 1500);
            }
          });
          fallbackPeer.on('error', (fErr: any) => {
            if (!isMountedRef.current) return;
            if (fErr?.type === 'peer-unavailable') {
              setErrorMessage('Kiritilgan xona kodi topilmadi yoki raqib oflayn.');
              setStatus('idle');
            }
          });
        } catch {}
      ...
      // Handle lost connection to signaling server silently with auto-reconnect
      if (
        errType === 'network' ||
  ...
  // Initialize Peer on mount and keep alive
  useEffect(() => {
    isMountedRef.current = true;
    initPeer();

    // Heartbeat check every 15s to keep signaling connection alive
  ...
  const setupConnectionHandlers = (conn: DataConnection) => {
    ...
    conn.on('error', () => {
      ...
      setStatus('idle');
      setOpponentConnected(false);
      setErrorMessage('Raqib bilan aloqa uzildi');
    });
  };
  ...
  const connectToOpponent = () => {
    ...
    try {
      ...
      setErrorMessage('Ulanishda xatolik yuz berdi. Xona kodini tekshiring.');
      setStatus('idle');
    }
  };
        ...
        <div className="flex items-center gap-2 p-2.5 rounded-2xl bg-white/[0.08] border border-white/15">
          ...
          <div className="flex items-center gap-1">
            ...
            <button
              ...
              title="Xonani yangilash / Qayta ulanish"
            >
